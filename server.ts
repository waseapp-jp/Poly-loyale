...
function broadcastRoomState(io: Server, room: GameState, isDeltaTick = false) {
  ...
  for (const human of humanPlayers) {
    ...
    if (human.inBus || (room.battleBus && room.battleBus.active)) {
      if (room.battleBus) {
    ...
    const effectiveRadius = human.isDead ? 130 : AOI_RADIUS;
    ...
    for (const pid in dynamicState.players) {
      ...
      if (Math.abs(p.x - fx) <= effectiveRadius && Math.abs(p.z - fz) <= effectiveRadius) {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('input', (input: ClientInput) => {
      ...
      if (!player.inBus && !player.isDead) {
        const groundH = getGroundHeight(player.x, player.z, room.obstacles);
        ...
        if (player.isSkydiving || player.isGliding) {
          if (player.y <= groundH + 1.2) {
            ...
            // While airborne, skip ground combat actions
            return;
          }
        }
      }
      ...
      if (room.status === 'waiting') {
        ...
        Object.values(room.players).forEach(p => {
          ...
          if (p.isFlying) {
            ...
            p.y = Math.min(groundH + 15, p.y + 10 * dt);
        ...
        alivePlayers.forEach(bot => {
          ...
          if (bombDanger) {
            ...
            if (targetP && targetDist < 35 && !isSeekingCover && bot.health >= bot.maxHealth * 0.4) {
              const isSightBlocked = isLineBlockedByObstacles(bot.x, bot.y + 1, bot.z, targetP.x, (targetP.y || 0) + 1, targetP.z, room.obstacles, roomId);
              if (isSightBlocked) {
